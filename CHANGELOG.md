...
### Changed
- Quadtree construction got a deterministic prefix-partition restructure: the serial build is 2.4x faster at N=100k (34.5 ms → 14.7 ms), and `--threads > 1` additionally schedules all leaf buckets across the rayon pool for a 4.7x threaded build vs the old serial (7.3 ms at 12 threads), with forces bitwise-identical to the previous build. Transient build allocations are charged against `--max-memory-mib`.
